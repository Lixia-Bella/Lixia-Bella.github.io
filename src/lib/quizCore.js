...
const QUESTION_BANK = [
	...
	},
	{
		...
		options: [
			'没有区别',
			'require 是 CommonJS，import 是 ESM',
			'import 更快',
			'require 已废弃',
		],
		answer: 1,
	...
	},
	{
		...
		options: [
			...
			'Cross-Object Rendering Service',
			'Cache-Origin Response Standard',
		],
		answer: 0,
	...
	},
	{
		...
		options: [
			'存储大小不同',
			'localStorage 持久化，sessionStorage 会话结束清除',
			'API 不同',
			'安全性不同',
		],
		answer: 1,
...
const handleAnswer = (selectedIndex) => {
	...
	if (selectedIndex === -1) {
		if (fIcon)
			fIcon.innerHTML =
				'<i class="fas fa-clock" style="color: var(--cream-yellow-dark);"></i>';
		if (fText) {
			fText.textContent = '时间到！';
			fText.className = 'feedback-text timeout';
		}
	} else if (isCorrect) {
		if (fIcon)
			fIcon.innerHTML =
				'<i class="fas fa-check-circle" style="color: var(--mint-green-dark);"></i>';
		if (fText) {
			fText.textContent = '回答正确！';
			fText.className = 'feedback-text correct';
		}
	} else {
		if (fIcon)
			fIcon.innerHTML =
				'<i class="fas fa-times-circle" style="color: var(--primary-pink-dark);"></i>';
		if (fText) {
			fText.textContent = '回答错误';
			fText.className = 'feedback-text wrong';
		}
	}
...
const showResult = () => {
	...
	const passed = correctCount >= currentLevel.passScore;
	let earnedPoints = correctCount * currentLevel.pointsPerCorrect;
	...
	if (
		...
		earnedPoints > gameData.levelRecords[levelKey].bestScore
	) {
		gameData.levelRecords[levelKey] = {
			...
			passed:
				passed ||
				(gameData.levelRecords[levelKey] && gameData.levelRecords[levelKey].passed),
		};
	}
	...
	const rAccuracy = document.getElementById('resultAccuracy');
	const rTime = document.getElementById('resultTime');
	const rPoints = document.getElementById('resultPoints');
	const rDetail = document.getElementById('resultDetail');

	if (passed) {
		if (rIcon)
			rIcon.innerHTML =
				'<i class="fas fa-trophy" style="color: var(--cream-yellow-dark);"></i>';
		if (rTitle) {
			rTitle.textContent = '恭喜通关！';
			rTitle.style.color = 'var(--mint-green-dark)';
		}
	} else {
		if (rIcon)
			rIcon.innerHTML =
				'<i class="fas fa-heart-broken" style="color: var(--primary-pink);"></i>';
		if (rTitle) {
			rTitle.textContent = '挑战失败，再接再厉！';
			rTitle.style.color = 'var(--primary-pink-dark)';
		}
	}
...
const renderLevels = () => {
	...
	const colorMap = {
		mint: {
			bg: 'rgba(168, 216, 200, 0.15)',
			border: 'rgba(168, 216, 200, 0.4)',
			icon: 'var(--mint-green-dark)',
		},
		pink: {
			bg: 'rgba(242, 166, 179, 0.15)',
			border: 'rgba(242, 166, 179, 0.4)',
			icon: 'var(--primary-pink-dark)',
		},
		lavender: {
			bg: 'rgba(195, 177, 225, 0.15)',
			border: 'rgba(195, 177, 225, 0.4)',
			icon: 'var(--lavender-dark)',
		},
		blue: {
			bg: 'rgba(167, 199, 231, 0.15)',
			border: 'rgba(167, 199, 231, 0.4)',
			icon: 'var(--sky-blue-dark)',
		},
		yellow: {
			bg: 'rgba(249, 228, 183, 0.2)',
			border: 'rgba(249, 228, 183, 0.5)',
			icon: 'var(--cream-yellow-dark)',
		},
	};
	...
	LEVELS.forEach((level) => {
		const colors = colorMap[level.color] || colorMap['pink'];
		const isUnlocked = gameData.totalPoints >= level.unlockPoints;
		const record = gameData.levelRecords[`level_${level.id}`];
		const isPassed = record && record.passed;
		...
		if (isPassed) {
			statusHtml =
				'<span class="level-status passed"><i class="fas fa-check-circle"></i> 已通关</span>';
		} else if (!isUnlocked) {
		...
		const maxPoints = level.questionCount * level.pointsPerCorrect + level.bonusPoints;
		card.innerHTML =
			...
			`</div>` +
			`<div class="level-reward"><i class="fas fa-coins"></i> 最高 ${maxPoints} 积分</div>` +
			statusHtml +
			bestHtml;
...
const renderRewards = () => {
	...
	const colorMap = {
		pink: {
			bg: 'linear-gradient(135deg, rgba(242,166,179,0.12), rgba(248,196,204,0.2))',
			border: 'rgba(242,166,179,0.35)',
			icon: 'var(--primary-pink)',
		},
		mint: {
			bg: 'linear-gradient(135deg, rgba(168,216,200,0.12), rgba(196,232,220,0.2))',
			border: 'rgba(168,216,200,0.35)',
			icon: 'var(--mint-green)',
		},
		lavender: {
			bg: 'linear-gradient(135deg, rgba(195,177,225,0.12), rgba(217,204,240,0.2))',
			border: 'rgba(195,177,225,0.35)',
			icon: 'var(--lavender)',
		},
		blue: {
			bg: 'linear-gradient(135deg, rgba(167,199,231,0.12), rgba(197,220,240,0.2))',
			border: 'rgba(167,199,231,0.35)',
			icon: 'var(--sky-blue)',
		},
		yellow: {
			bg: 'linear-gradient(135deg, rgba(249,228,183,0.15), rgba(253,240,213,0.25))',
			border: 'rgba(249,228,183,0.4)',
			icon: 'var(--cream-yellow-dark)',
		},
	};
	...
	REWARD_POOLS.forEach((reward) => {
		const colors = colorMap[reward.color] || colorMap['pink'];
		const stock = getRewardStock(gameData, reward.id);
		const canExchange = gameData.totalPoints >= reward.requiredPoints && stock > 0;
		...
		const stockText = isSoldOut ? '已兑完' : `剩余 ${stock} 个`;
		let btnContent;
		if (isSoldOut) {
			btnContent = '已兑完';
		} else if (canExchange) {
			btnContent = '<i class="fas fa-exchange-alt"></i> 立即兑换';
		} else {
			btnContent = '<i class="fas fa-lock"></i> 积分不足';
		}
		...
		card.innerHTML =
			...
			`<span class="reward-stock">${stockText}</span>` +
			`</div>` +
	...
	grid.querySelectorAll('.btn-exchange:not(.disabled)').forEach((btn) => {
...
/**
 * React 路由离开时清理答题计时器，避免后台仍触发 handleAnswer
 */
export const destroyQuizTimers = () => {
	if (timerInterval) {
		clearInterval(timerInterval);
		timerInterval = null;
	}
